...
def menus_action():
    ...
    try:
        ...
        if action == 'add':
            ...
            menus_id = request.form.get('menu_id')
            food_name = request.form.get('name')
            food_type = request.form.get('food_type')
            cuisine = request.form.get('cuisine')
            price = request.form.get('price')
            ...
            if not any([menus_id, food_name, food_type, cuisine, price, restaurant_id]):
            ...
            session['filtered_menus'] = menus
            flash(f"Found {len(menus)} menus item(s) matching the criteria(s).", "success")
